...
static void do_request(std::vector<std::string> &cmd, Response &out)
{
  ...
  {
    // get from map and return
    auto it = g_data.find(cmd[0]);
    if (it == g_data.end())
    {
      out.status = RES_NX;
    }
...
static void make_reponse(const Response &resp, std::vector<uint8_t> &out)
{
  uint32_t resp_len = 4 + (uint32_t)out.size();
